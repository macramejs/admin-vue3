<template>
    ...
    >
        <div class="relative w-full">
            <ComboboxInput
                :displayValue="(item: any) => item[props.labelKey]"
                    ...
                    <label
                        ...
                        :class="{
                            'text-ellipsis !-translate-y-[35px] !scale-100 text-indigo-900':
                                selectedItems.length > 0 || isFocused,
            ...
            <span
                v-if="selectedItems.length > 0 && query == '' && !disabled"
                    ...
                    >
                        <li
                            :class="[
                                ...
                                active ? 'bg-gray-100' : '',
                                'cursor-pointer select-none hover:bg-gray-200 flex items-center px-4 justify-between relative',
                            ]"
                        >
                            <span
                                :class="[
                            ...
                            <div
                                v-if="selected"
                                class="flex items-center justify-center w-4 h-4 text-white rounded-full bg-green"
                            >
...
const itemLabel = computed(() => {
    ...
    if (props.valueKey) {
        let selectedOptionsX: any = props.options.find(
            (option: any) => option[props.valueKey] == props.modelValue
        );

        let selectedOptions = [];
        for (let i = 0; i < props.modelValue.length; i++) {
            const element = props.modelValue[i];
            selectedOptions.push(
                props.options.find(
                    (option: any) => option[props.valueKey] == element
                )
            );
        }
        ...
        let output = selectedOptions.join(', ');

        return selectedOptions ? output : props.modelValue.join(', ');
    }
    return props.modelValue.join(', ');
});

const selectedItems = ref<SelectedType>(props.modelValue);
const query = ref('');
